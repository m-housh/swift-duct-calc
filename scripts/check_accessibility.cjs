...
(async () => {
  ...
  try {
    ...
    const templateTrigger = dialog.locator('[data-trunk-template-toggle]');
    assert.equal(await dialog.locator('[data-trunk-template-summary]').textContent(), 'No template');
    const modalBounds = await dialog.locator('.modal-box').boundingBox();
    await templateTrigger.click();
    assert.deepEqual(await dialog.locator('.modal-box').boundingBox(), modalBounds, 'Dropdown must not resize the modal');
    await audit('Trunk template dropdown', true);
    await page.keyboard.press('Escape');
    await expect(dialog).toBeVisible();
    await focused(templateTrigger);
    await page.keyboard.press('ArrowDown');
    await focused(dialog.getByRole('button', { name: 'No template', exact: true }));
    await page.keyboard.press('ArrowDown');
    await page.keyboard.press('ArrowRight');
    await page.keyboard.press('Enter');
    assert.equal(await dialog.locator('[name=type]').inputValue(), 'return');
    assert.equal(await dialog.locator('input[name=rooms]:checked').count(), 2);
    await fill({ height: 8 }, dialog);
    await templateTrigger.click();
    await dialog.getByRole('button', { name: 'No template', exact: true }).click();
    assert.equal(await dialog.locator('[name=name]').inputValue(), '');
    assert.equal(await dialog.locator('[name=height]').inputValue(), '8');
    assert.equal(await dialog.locator('[name=type]').inputValue(), 'return');
