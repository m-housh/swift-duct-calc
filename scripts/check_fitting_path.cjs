...
(async()=>{const b=await chromium.launch({headless:true,args:['--no-sandbox']});try{
 ...
 await offset.locator('[data-favorite]').click();await p.waitForFunction(()=>document.querySelector('[data-catalog-id="8O"] [data-favorite]').getAttribute('aria-pressed')==='true');await offset.locator('[data-row]').click();await p.waitForFunction(()=>document.querySelectorAll('#path-rows [data-row-id]').length===2);
 ...
 await group(8);assert.equal(await p.locator('[data-favorite-copy="8O"]').count(),1);assert.equal(await p.locator('[data-catalog-id="8O"] [data-favorite]').getAttribute('aria-pressed'),'true');await p.locator('[data-close-dialog="picker-dialog"]').click();
