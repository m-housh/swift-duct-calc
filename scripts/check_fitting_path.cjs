...
(async()=>{const b=await chromium.launch({headless:true,args:['--no-sandbox']});try{
 ...
 await p.goto(base+'/editor');await p.locator('#path-name').fill('Living room supply');await p.locator('#path-straight').fill('10, 25');
 ...
 await p.locator('#save-path').click();await p.waitForURL(/editor\?id=/);assert.equal(await p.locator('#path-total').innerText(),total);let saved=JSON.parse(await p.locator('#fitting-path').getAttribute('data-baseline'));assert.equal(saved.groups[2].value,61.375);assert.equal(saved.groups[0].quantity,2);assert.equal(saved.groups[1].fitting.origin,'catalog');
 fs.writeFileSync('/tmp/fitting-review-path.txt',p.url());
 await p.locator('#path-name').fill('Living room supply reviewed');await p.locator('#save-path').click();await p.waitForFunction(()=>JSON.parse(document.querySelector('#fitting-path').dataset.baseline).name==='Living room supply reviewed');assert.equal(await p.locator('#path-total').innerText(),total);
 ...
 await p.locator('#save-path').click();await p.waitForFunction(()=>JSON.parse(document.querySelector('#fitting-path').dataset.baseline).groups.length===4);
