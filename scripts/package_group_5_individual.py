#!/usr/bin/env python3
"""Package individual Group 5 fittings while preserving the full source artwork."""
import argparse
import base64
import hashlib
import json
...
PUBLIC = ROOT / 'Public'
OUT = PUBLIC / 'images/fittings/group-5-individual'
QA = ROOT / 'docs/fitting-preflight/group-5-individual.json'
...
def package(activate=False):
    original = json.loads((PUBLIC / 'images/fittings/group-5-enhanced/manifest.json').read_text())
    ...
    if len(checks) != len(qa['items']) or set(checks) != expected:
        raise ValueError('Individual Group 5 QA must cover every fitting exactly once.')
    ...
    for path in (ROOT / 'docs/fitting-reviews').glob('*.json'):
        ...
        if report.get('batchId') == 'group-5-individual' and report.get('completedAt'):
    ...
    for source in original['items']:
        ...
        items.append(item)
        src = source['source']
        cards.append({'id': item['id'], 'number': item['fittingNumber'], 'name': item['name'], 'group': 5,
            'image': '..' + item['svg'] + '?revision=' + item['revision'][:12],
            'reference': '..' + item['referenceImage'], 'sourcePage': src['printedPage'],
            'sourcePDF': f"..{src['pdf']}#page={src['pdfPage']}", 'revision': item['revision'],
            'contextURL': 'group-5-references.html#' + ref['id'], 'notes': check['notes'],
            'reviewNote': ('Original standalone fitting views retained.' if item['unchangedStandalone'] else
                'Only this fitting connection is shown. Original plenum and equipment context retained in both views.'),
            'priorApproval': accepted})
    accepted = all(item['status'] == 'visually-approved' for item in items)
    write_json(OUT / 'manifest.json', {'schemaVersion': 1, 'group': 5, 'title': 'Group 5 individual fittings',
    ...
    batch = {'id': 'group-5-individual', 'title': 'Group 5 · individual fittings',
        'description': 'Each entry shows its own fitting. 5A–5G are separated from shared assemblies; '
        '5H–5O retain their original standalone views. Full source illustrations remain available for comparison.',
        'referenceGallery': 'group-5-references.html', 'items': cards}
    write_json(PUBLIC / 'fitting-review/batches/group-5-individual.json', batch)
    gallery = '''<!doctype html><html lang="en"><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Group 5 full references</title>
...
<header><a href="./">Back to fitting review</a><h1>Group 5 full references</h1>
<p>Preserved full illustrations for comparison with the individual fittings.</p></header><main>'''
    ...
    (PUBLIC / 'fitting-review/group-5-references.html').write_text(gallery + '</main></html>\n')
    if activate:
        (PUBLIC / 'fitting-review/data.js').write_text('window.FITTING_REVIEW = ' + json.dumps(batch, indent=2) + ';\n')
    print(f'Group 5 individual: {len(items)} drawings, {len(references)} full references; activated={activate}')
