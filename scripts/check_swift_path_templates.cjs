...
(async () => {
  ...
  const browsed = await mount(html, http);
  await browsed.click('choose', '[data-id="1B"]');
  await browsed.click('browse');
  assert.equal(browsed.root.querySelector('#browse-group').value, '2');
  await browsed.click('extra', '[data-id="2C"]');
  await browsed.click('apply-details');
  assert(browsed.root.querySelector('#detail-error').textContent.includes('Complete'));
  assert.equal(browsed.heading(), 'Supply trunk branch takeoff');
  await browsed.click('cancel-details');
  assert.equal(browsed.root.querySelector('[data-action="choose"][data-id="2C"]'), null);
  await browsed.click('browse');
  await browsed.click('extra', '[data-id="2C"]');
  await browsed.set('[data-field="count"]', '0');
  await browsed.submitDetails();
  assert.equal(browsed.heading(), 'Boot', 'Browsed choose-one fitting must advance');
  await browsed.click('back');
  assert(browsed.root.querySelector('[data-action="choose"][data-id="2C"]'));
  assert.equal(browsed.root.querySelectorAll('[data-action="edit-row"]').length, 1);
  await browsed.click('browse');
  await browsed.set('#browse-group', '1');
  await browsed.click('extra', '[data-id="1C"]');
  assert.equal(browsed.heading(), 'Supply trunk branch takeoff', 'Another group must not complete this section');
  assert.equal(browsed.root.querySelectorAll('[data-action="edit-row"]').length, 1);
  await browsed.click('choose', '[data-id="2C"]');
  assert.equal(browsed.heading(), 'Boot', 'Browsed inputs must survive revisits');
  await browsed.click('choose', '[data-id="4G"]');
  await browsed.click('browse');
  assert.equal(browsed.root.querySelector('#browse-group').value, '8');
  await browsed.click('extra', '[data-id="8A-2-piece-45"]');
  assert.equal(browsed.heading(), 'Elbows', 'Quantity sections must allow further entries');
  assert.equal(browsed.root.querySelector('[data-quantity="8A-2-piece-45"]').value, '1');
  await browsed.set('[data-quantity="8A-2-piece-45"]', '3');
  await browsed.click('browse');
  await browsed.click('extra', '[data-id="8A-2-piece-45"]');
  assert.equal(browsed.root.querySelector('[data-quantity="8A-2-piece-45"]').value, '3');
  assert.equal(browsed.root.querySelectorAll('[data-action="edit-row"]').length, 1);
  await browsed.click('done');
  await browsed.click('browse');
  assert.equal(browsed.root.querySelector('#browse-group').value, '12');
  await browsed.click('extra', '[data-id="12V"]');
  assert.equal(browsed.heading(), 'Transitions', 'Multiple-choice sections must allow further entries');
  assert(browsed.root.querySelector('[data-action="choose"][data-id="12V"]'));
  await browsed.click('done');
  await browsed.set('[data-path="name"]', 'QA browsed');
  await browsed.click('save-path');
  assert.equal(browsed.redirects.at(-1), `/projects/${projectID}/effective-lengths`);
  const browsedList = new JSDOM(await (await http(browsed.redirects.at(-1))).text()).window.document;
  const browsedURL = [...browsedList.querySelectorAll('tr')]
    .find(row => row.textContent.includes('QA browsed')).querySelector('a[href*="/editor?id="]').getAttribute('href');
  const browsedEditor = new JSDOM(await (await http(browsedURL)).text()).window.document.querySelector('#fitting-path');
  const browsedPath = JSON.parse(browsedEditor.dataset.baseline);
  assert.deepEqual(browsedPath.templateSnapshot.configuration, data.configuration);
  assert.equal(browsedPath.groups.find(row => row.calculation.fittingID === '2C').stepID, data.configuration.steps[1].id);
  assert(browsedPath.groups.some(row => row.calculation.fittingID === '1C' && !row.stepID));
  assert.equal(browsedPath.groups.find(row => row.calculation.fittingID === '8A-2-piece-45').quantity, 3);
  assert.equal(JSON.parse(browsedEditor.dataset.rows).length, browsedPath.groups.length);
  assert.deepEqual(payload(await (await http(`/path-templates/${data.template.id}`)).text()).configuration, data.configuration);
  browsed.dom.window.close();
