...
(async () => {
  ...
  try {
    ...
    if (await page.locator('#catalog-grid [data-favorite="1D"]').getAttribute('aria-pressed') !== 'true') await page.locator('#catalog-grid [data-favorite="1D"]').click();
    await page.waitForFunction(() => document.querySelector('#catalog-grid [data-favorite="1D"]').getAttribute('aria-pressed') === 'true');
    ...
    assert.equal(await cards.first().getAttribute('data-catalog-id'), '1C');
    assert.equal(await cards.count(), count);
    await select('none');
    assert.equal(await cards.first().getAttribute('data-catalog-id'), '1A', 'No preference restores source order');
    await select('round');
    ...
    const favorite2A = page.locator('#catalog-grid [data-favorite="2A"]');
    const wasFavorite = await favorite2A.getAttribute('aria-pressed') === 'true';
    if (!wasFavorite) await favorite2A.click();
    await page.waitForFunction(() => document.querySelector('#catalog-grid [data-favorite="2A"]').getAttribute('aria-pressed') === 'true');
    ...
    assert.equal(await cards.first().getAttribute('data-catalog-id'), '8B', 'Favorites do not reorder the catalog');
    assert.equal(await page.locator('[data-favorite-copy="8O"]').count(), 1);
    await select('round');
    await group(2);
