#!/usr/bin/env python3
"""Package individually reviewed Group 8 source restorations and picker variants."""
import argparse
import hashlib
import json
...
def package(activate=False):
    out = PUBLIC / 'images/fittings/group-8'
    source = json.loads((out / 'source-metadata.json').read_text())
    qa_path = ROOT / 'docs/fitting-preflight/group-8.json'
    ...
    for path in (ROOT / 'docs/fitting-reviews').glob('*.json'):
        ...
        if report.get('batchId') == 'group-8' and report.get('completedAt'):
    ...
    for original in source['items']:
        ...
        if item['fittingId'] in {'8L', '8M'}:
            family.setdefault('artworkByShape', {})[item['ductShape']] = {'itemId': item_id, 'image': item['svg']}
        src = item['source']
        cards.append({'id': item_id, 'number': item_id, 'name': item['name'], 'group': 8,
                      ...
                      'contextURL': 'group-8-references.html#page-' + str(src['printedPage']),
                      'notes': [note], 'reviewNote': note, 'priorApproval': accepted})
    write_json(out / 'manifest.json', {'schemaVersion': 1, 'group': 8, 'title': source['title'],
        ...
        'items': items})
    batch = {'id': 'group-8', 'title': 'Group 8 · elbows and offsets',
             'description': 'Individual source fittings 8A–8P. 8A construction styles and the round/rectangular '
                            '8L–8M alternatives have separate artwork. Full source tables remain linked.',
             'referenceGallery': 'group-8-references.html', 'items': cards}
    write_json(PUBLIC / 'fitting-review/batches/group-8.json', batch)
    gallery = '''<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Group 8 original references</title><link rel="stylesheet" href="review.css">
<style>article{padding:24px}img{display:block;max-width:100%;height:auto;margin:auto}</style>
<header><a href="./">Back to fitting review</a><h1>Group 8 original references</h1></header><main>'''
    for page in source['sourcePages']:
        gallery += f'<article id="page-{page}"><h2>Source page {page}</h2><img src="../images/fittings/group-8/source-art/page-{page}.png" alt="Original Group 8 drawings and tables, page {page}"></article>'
    (PUBLIC / 'fitting-review/group-8-references.html').write_text(gallery + '</main></html>\n')
    if activate:
        (PUBLIC / 'fitting-review/data.js').write_text('window.FITTING_REVIEW = ' + json.dumps(batch, indent=2) + ';\n')
    print(f'Group 8: {len(items)} drawings in {len(families)} families; activated={activate}')
