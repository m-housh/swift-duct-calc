...
struct FittingsRouteTests {
  ...
  func publicReferenceAndProtectedProjects() async throws {
    try await withApp(configure: configuredApp) { app in
      ...
      let response = try await client.sendRequest(.GET, "/fittings?system=return&group=8&fitting=8A-smooth&data=json")
  ...
  func loginRestoresUserOnPublicPagesAndLogoutClearsIt() async throws {
    try await withApp(configure: configuredApp) { app in
      ...
      let user = try await database.users.create(.init(email: "reference@example.com", password: "super-secret", confirmPassword: "super-secret"))
      _ = try await database.userProfiles.create(.init(
        userID: user.id, firstName: "Reference", lastName: "Reader", companyName: "Reference Co",
        streetAddress: "123 Main St", city: "Cincinnati", state: "OH", zipCode: "45202", theme: .nord
      ))
      let client = try app.testing()
      let login = try await client.sendRequest(.POST, "/login", headers: ["Content-Type": "application/x-www-form-urlencoded"], body: .init(string: "email=reference%40example.com&password=super-secret&next=%2Ffittings%3Fsystem%3Dreturn%26data%3Djson"))
  ...
  func deletedUserSessionFallsBackToGuest() async throws {
    try await withApp(configure: configuredApp) { app in
      let database = DatabaseClient.live(database: app.db)
      let user = try await database.users.create(.init(email: "deleted@example.com", password: "super-secret", confirmPassword: "super-secret"))
      let client = try app.testing()
      let login = try await client.sendRequest(.POST, "/login", headers: ["Content-Type": "application/x-www-form-urlencoded"], body: .init(string: "email=deleted%40example.com&password=super-secret"))
      let cookie = try #require(login.headers.first(name: .setCookie)).split(separator: ";")[0]
  ...
  func htmxContinuationLoadsTheWholeDocument() async throws {
    try await withApp(configure: configuredApp) { app in
      ...
      let response = try await app.testing().sendRequest(.GET, path, headers: ["HX-Request": "true"])
      #expect(response.status == .ok)
  ...
  func queryStateRoundTrips() throws {
    let route = SiteRoute.View.fittings(.init(system: "supply", group: "8", fitting: "8A-smooth", q: "round & smooth", type: "conditional", data: "json"))
    ...
    #expect(try SiteRoute.View.router.parse(URLRequestData(string: "/fittings")!) == .fittings(.init()))
  }
}
