...
struct PathTemplateTests {
  ...
  func everyEligibleGroupCanBeConfigured() throws {
    for type in EquivalentLength.EffectiveLengthType.allCases {
      let steps = Fitting.Group.allCases.filter { $0.supports(type) }.map { group in
        PathTemplate.Step(
  ...
  func unansweredCountRemainsDistinctFromZero() throws {
    let unanswered = Fitting.Inputs.downstreamBranches(nil)
    let zero = Fitting.Inputs.downstreamBranches(0)
    #expect(unanswered != zero)
    #expect(
      try JSONDecoder().decode(Fitting.Inputs.self, from: JSONEncoder().encode(unanswered))
        == unanswered
    )
    #expect(
      try JSONDecoder().decode(Fitting.Inputs.self, from: JSONEncoder().encode(zero)) == zero
    )
  }
}
