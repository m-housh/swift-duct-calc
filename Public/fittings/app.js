(() => {
  ...
  function conditions(f) {
    ...
    return entries.length ? `<dl class="conditions">${entries.map(([k,v]) => `<div><dt>${esc(labels[k] || k.replace(/([A-Z])/g, ' $1').replace(/^./, x => x.toUpperCase()))}</dt><dd>${esc(readable(v))}</dd></div>`).join('')}</dl>` : '<p class="muted">See the source table for reference conditions.</p>';
  }
  function referenceTable(f) {
    const t = R.table(f);
    if (!t.rows.length) return '<p class="notice">No supported reference table is available. Read the source before choosing a value.</p>';
