...
(() => {
  ...
  function initialize() {
    ...
    function fittingCard(d, action) {
      ...
      )}${unavailable ? '<p class="text-warning">Calculation awaiting source verification.</p>' : ''}${source(d)}</article>`;
    }
    function source(d) {
      ...
      switch (kind(d)) {
        ...
        case 'sourceTable':
          return d.requirements.sourceTable.axes
            ...
            .join('');
        default:
          return '<p class="text-warning">Calculation awaiting source verification.</p>';
      }
    }
          ...
          <ol class="space-y-4">${configuration.steps
            ...
            ${step.choices
              .map((choice) => {
                ...
                ${kind(d) === 'unavailable' ? '<p class="text-warning">Automatic calculation is unavailable for this fitting. It will remain in the template.</p>' : ''}
