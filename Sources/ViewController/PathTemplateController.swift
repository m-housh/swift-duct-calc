...
private func allFittings() async throws -> [Fitting.Definition] {
  @Dependency(\.fittingClient) var fittings
  ...
  func renderView(on request: ViewController.Request) async -> AnySendableHTML {
    ...
    do {
      ...
      if let projectID {
        ...
        guard try await projects.getOwned(user.id, projectID) != nil else { throw NotFoundError() }
      }
      switch self {
      case .index:
...
extension SiteRoute.View.ProjectRoute.EquivalentLengthRoute.GuidedRoute {
    ...
    -> AnySendableHTML
  {
    ...
    do {
      let user = try request.currentUser()
      guard try await database.projects.getOwned(user.id, projectID) != nil else {
        throw NotFoundError()
      }
      switch self {
      case .index:
...
private func workspaceError(_ error: any Error) -> some HTML & Sendable {
  ...
  if let error = error as? FittingClient.TemplateValidationError {
