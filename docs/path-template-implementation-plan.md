...
opens the editor; Equivalent Lengths → Plus → From template starts a guided path.
...
The catalog contains 231 definitions and 47 enabled calculation cases, covering
every starter-template choice. Other cases remain configurable and show that
automatic calculation is unavailable. The
[source audit](fitting-rule-audit.md) records the enabled rules and remaining work.

The delivery plan below records the agreed behavior and architecture. Source
expansion, browser visual review across all themes and screen sizes, and the
broader quick-entry/favorites work from the fitting-picker plan remain separate
follow-up work.
...
- Use the Equivalent Lengths plus button for both entry methods. Its modal starts
  with "From template", followed by the existing manual name/type form. Show this
  choice only when creating a path, and keep template management available.
...
| --- | --- |
| `ManualDCore/Fittings.swift` | Shared fitting IDs, typed inputs, definitions, defaults, results, and issues |
| `ManualDCore/PathTemplate.swift` | User template, ordered steps, behavior, allowed choices, and typed input defaults |
| `FittingClient` | Catalog eligibility, artwork, input requirements, source-backed evaluation, and availability |
...
owned by `FittingClient`; controllers use the same validation for preview and save.
